...
from credibility_experiments import CustomMapFrozenLakeEnv, evaluate_on_env, make_random_map
...
def run_designed_16x16(args: argparse.Namespace, out: Path) -> None:
    out.mkdir(parents=True, exist_ok=True)
    (out / "figures").mkdir(exist_ok=True)
    (out / "tables").mkdir(exist_ok=True)
    ...
    maps = select_designed_maps(
        size=16,
        n_holes=args.holes16,
        n_maps=args.maps16,
        min_oracle=args.min_oracle16,
        max_oracle=args.max_oracle16,
        eval_episodes=args.oracle_eval_episodes16,
        max_steps=args.max_steps16,
        seed=args.seed16,
    )
    maps.to_csv(out / "tables" / "designed_16x16_map_manifest.csv", index=False)

    cfg = base.ExperimentConfig(
        episodes=args.episodes16,
        eval_episodes=args.eval_episodes16,
        max_steps=args.max_steps16,
        eval_interval=args.episodes16,
    )
    cfg.planning_steps = args.planning_steps16
    cfg.optimistic_init = args.optimistic_init

    algorithms = [item.strip() for item in args.algorithms16.split(",") if item.strip()]
    rows = []
    for _, m in maps.iterrows():
        layout = str(m["layout"]).split("|")
        for algorithm in algorithms:
            for seed in range(args.seeds16):
                env = CustomMapFrozenLakeEnv(layout, True, seed)
                agent = train_agent(algorithm, env, cfg, seed)
                ...
                rows.append(
                    {
                        "suite": "designed_16x16_exact_holes",
                        "algorithm": algorithm,
                        "display_algorithm": DISPLAY.get(algorithm, algorithm),
                        "env": f"16x16_designed_stoch_exact{args.holes16}holes",
                        "map_id": int(m["map_id"]),
                        "seed": seed,
                        "success": score,
                        "oracle_success": float(m["oracle_success"]),
                ...
                pd.DataFrame(rows).to_csv(out / "tables" / "designed_16x16_runs_partial.csv", index=False)
                print("designed16x16", m["map_id"], algorithm, seed, score, "oracle", m["oracle_success"])
    ...
    runs.to_csv(out / "tables" / "designed_16x16_runs.csv", index=False)
    aggregate.to_csv(out / "tables" / "designed_16x16_aggregate_summary.csv", index=False)
    ...
    ax.set_title(f"Designed 16x16 Slippery Maps: Exact {args.holes16} Holes")
    ax.set_xlabel("Designed map id")
    ax.set_ylabel("Success rate")
    ax.set_ylim(0, 1.05)
    ax.grid(axis="y", alpha=0.25)
    ax.legend(loc="upper left", bbox_to_anchor=(1.01, 1.0), fontsize=8, frameon=False)
    fig.tight_layout()
    fig.savefig(out / "figures" / "designed_16x16_exactholes_success_by_map.png", dpi=220)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(8.2, 4.8))
    sns.barplot(data=aggregate, x="display_algorithm", y="mean_success", errorbar=None, ax=ax)
    ax.axhline(float(maps["oracle_success"].mean()), color="black", linestyle="--", linewidth=2.0, label="Mean oracle")
    ax.set_title(f"Designed 16x16 Exact-{args.holes16}-Hole Maps: Aggregate")
    ax.set_xlabel("")
    ax.set_ylabel("Mean success rate")
    ax.set_ylim(0, 1.05)
    ax.tick_params(axis="x", rotation=25)
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig(out / "figures" / "designed_16x16_exactholes_aggregate.png", dpi=220)
    plt.close(fig)
...
def audit_maps(out: Path, designed_dir: Path | None = None) -> None:
    ...
    for name, layout, source in [
        ...
        ("current_fixed_16x16_p0.85", base.make_map(16, 2026, 0.85), "Project generated fixed map"),
    ]:
        rows.append(
            {
                "map_name": name,
    ...
    if designed_dir is not None and (designed_dir / "tables" / "designed_16x16_map_manifest.csv").exists():
        manifest16 = pd.read_csv(designed_dir / "tables" / "designed_16x16_map_manifest.csv")
        for _, row in manifest16.iterrows():
            rows.append(
                {
                    "map_name": f"designed_16x16_map_{int(row['map_id'])}",
                    "source": "Exact-hole designed 16x16 random",
                    "size": int(row["size"]),
                    "holes": int(row["holes"]),
                    "hole_ratio": int(row["holes"]) / (int(row["size"]) ** 2),
                    "shortest_path": int(row["shortest_path"]),
    ...
    for p in [0.90, 0.92, 0.95]:
        for map_id in range(3):
            map_seed = 50_000 + int(p * 100) * 100 + map_id
            layout = make_random_map(16, p, map_seed)
            rows.append(
                {
                    "map_name": f"sample_16x16_oracle_filter_p{p:.2f}_map_{map_id}",
                    "source": "Current 16x16 oracle-filter candidate generator",
                    "size": 16,
                    "holes": hole_count(layout),
                    "hole_ratio": hole_count(layout) / 256,
                    "shortest_path": shortest_path_length(layout),
                    "layout": "|".join(layout),
                }
            )
    ...
    markdown = [
        ...
        "- Current 16x16 generated map is a stress-test setup, not a final benchmark.",
        "- Designed 16x16 maps use exact hole-count and oracle filtering as the next controlled large-map step.",
        "",
        "## Layouts",
        "",
    ]
...
def main() -> None:
    ...
    parser.add_argument("--maps16", type=int, default=2)
    parser.add_argument("--holes16", type=int, default=38)
    parser.add_argument("--episodes16", type=int, default=1200)
    parser.add_argument("--eval-episodes16", type=int, default=100)
    parser.add_argument("--oracle-eval-episodes16", type=int, default=150)
    parser.add_argument("--max-steps16", type=int, default=500)
    parser.add_argument("--seeds16", type=int, default=1)
    parser.add_argument("--min-oracle16", type=float, default=0.25)
    parser.add_argument("--max-oracle16", type=float, default=0.90)
    parser.add_argument("--planning-steps16", type=int, default=3)
    parser.add_argument("--seed16", type=int, default=64_000)
    parser.add_argument("--algorithms16", default="q_learning,optimistic_q,sarsa_lambda,dyna_q")
    ...
    if "designed16" in suites:
        run_designed_16x16(args, args.output_dir)
    if "audit" in suites:
