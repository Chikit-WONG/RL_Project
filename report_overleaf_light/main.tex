...
Figure~\ref{fig:propagation} is not intended as a second overall algorithm ranking. It isolates backup and reward-propagation mechanisms on the fixed 8$\times$8 map. n-step SARSA is included because it is the canonical multi-step TD control baseline between one-step SARSA and eligibility traces. Optimistic Q-learning is not included in this panel because its main change is exploration through optimistic initialization, which is already tested in Figure~\ref{fig:explore}. Under this fixed-map ablation, one-step Q-learning is strongest at 0.760, while Q($\lambda$), Dyna-Q, SARSA($\lambda$), and n-step SARSA are lower or more variable. This does not contradict the main controlled-random result; it shows that propagation tricks alone do not dominate unless the exploration problem is also handled well.
...
The ablation results support this interpretation. The exploration ablation shows that optimistic initialization is more effective than UCB-Q or softmax exploration in this tabular FrozenLake setting. The reward-propagation ablation answers a separate question: whether multi-step backups, eligibility traces, or model-based planning improve learning once reward has been discovered. These mechanisms help in some cases, especially in AUC or convergence behavior, but they do not replace the need for effective exploration.
