...
\title{\vspace{-1.2em}\textbf{Benchmarking Tabular Reinforcement Learning on Stochastic FrozenLake}}
...
\begin{abstract}
This project extends an initial 4$\times$4 Q-learning implementation into a stochastic FrozenLake benchmark. We focus on the slippery setting, where the executed action may deviate from the intended action, because this setting tests reinforcement learning under transition uncertainty rather than deterministic maze following. The benchmark compares classical tabular control methods including Q-learning, SARSA, Expected SARSA, SARSA($\lambda$), Q($\lambda$), Dyna-Q, and optimistic Q-learning, with Value Iteration as a known-model oracle. A central part of the project is the evolution of the experimental design. We first used Gymnasium as a reference point: the standard Gymnasium 8$\times$8 map is a strong benchmark and sanity check, while Gymnasium-generated low-$p$ random maps revealed a problem--many are connected but nearly impossible under slippery dynamics. We therefore introduced controlled maps: standard 4$\times$4, designed 4$\times$4 maps with exactly four holes, fixed 8$\times$8, and controlled random 8$\times$8 maps. These maps are solvable, non-trivial, reproducible, and better suited for comparing algorithms. On the controlled random 8$\times$8 benchmark, optimistic Q-learning reaches 0.988 mean final success, followed by Expected SARSA and SARSA above 0.89. The Gymnasium standard 8$\times$8 result is retained as an important standard comparison, and the Gymnasium random-map results are retained as a diagnostic motivation for careful map design.
\end{abstract}

\section{Introduction}
FrozenLake is a compact reinforcement learning (RL) benchmark in which an agent starts on a grid, avoids holes, and tries to reach a goal. Rewards are sparse: the agent receives reward 1 only at the goal and 0 elsewhere. In the slippery version, an intended move can slip to a neighboring direction, so even an optimal policy cannot be interpreted as a deterministic path-following solution.

The original project implemented Q-learning on 4$\times$4 FrozenLake. That was a useful baseline, but it left three open questions: whether other tabular RL algorithms perform differently, whether results hold beyond a single small map, and whether hyperparameter choices are credible. This report addresses those questions with a stochastic-only benchmark, controlled map design, and focused ablation studies. The final benchmark was not chosen blindly: it was shaped by the contrast between the useful Gymnasium standard 8$\times$8 benchmark and the often near-impossible Gymnasium low-$p$ random maps.

The research questions are:
\begin{enumerate}
    \item Which tabular RL methods are strongest on slippery FrozenLake?
    \item Why is controlled map design necessary for fair algorithm comparison on stochastic FrozenLake?
...
The experiment was developed in two stages. We first used Gymnasium as the reference implementation. The standard Gymnasium 8$\times$8 map is valuable because it is widely recognized, reproducible, and neither trivial nor impossible under \texttt{slippery=True}. It therefore serves as an important standard benchmark and sanity check for the training pipeline.

The problem appeared when we moved from the standard Gymnasium map to library-generated random maps. Gymnasium's random-map generator guarantees that a path exists from start to goal, but in slippery FrozenLake deterministic reachability is not enough. A map can contain a valid path while still having very low optimal success probability, because slips can repeatedly push the agent into holes. In our Gymnasium random-map trial, many $p=0.5$ and $p=0.6$ maps produced near-zero oracle success and near-zero learned success for every algorithm. Such maps are useful as stress tests, but they are not useful for ranking algorithms because the dominant factor is map impossibility rather than learning quality.

For this reason, the final comparison benchmark uses carefully designed maps. The design goal is not to make the problem easy, but to make it comparable: each map should be stochastic, solvable, non-trivial, and reproducible. We therefore keep standard maps, especially the Gymnasium standard 8$\times$8 map, for baseline comparability; add controlled 4$\times$4 maps with exactly four holes; and use controlled 8$\times$8 random maps for the main scalability comparison. The Gymnasium random-map results are retained to document why this design choice was necessary.
...
We evaluate four stochastic settings:
\begin{itemize}
...
The controlled 4$\times$4 design prevents a different failure mode: unconstrained high-$p$ random generation can create trivial maps with too few holes. For 8$\times$8, the controlled random maps are appropriate for comparing algorithms because they produce solvable stochastic tasks with meaningful variation. The Gymnasium maps are reported separately because several low-$p$ maps have very low oracle success under slippery dynamics; mixing them directly with the controlled maps would make the mean score mainly reflect map impossibility rather than algorithm quality.

\subsection{Algorithms and Metrics}
The main comparison uses seven learned tabular methods: Q-learning, SARSA, Expected SARSA, SARSA($\lambda$), Q($\lambda$), Dyna-Q, and optimistic Q-learning. The exploration ablation additionally compares UCB-Q and softmax Q-learning. Value Iteration is used as an oracle, not as a fair model-free competitor, because it uses the known transition model.

The primary metric is greedy evaluation success rate after training. We also report standard deviation across seeds or maps and compare learned policies against the Value Iteration oracle. A high oracle score with a low learned score means the map is solvable but the learning algorithm failed to discover or propagate reward efficiently.

\section{Results}
\subsection{Standard and Controlled 4x4}
...
The Gymnasium 8$\times$8 results have two different roles. First, the standard Gymnasium 8$\times$8 map is an important benchmark. It is widely used, reproducible, and challenging without being degenerate. In our run, its oracle success is 0.861, and the learned mean success across the seven algorithms is 0.709. Several algorithms learn strong policies on this map: Expected SARSA reaches 0.85, Q($\lambda$) reaches 0.85, optimistic Q-learning reaches 0.82, Q-learning reaches 0.78, SARSA($\lambda$) reaches 0.76, and SARSA reaches 0.74. This confirms that the training pipeline can handle a standard stochastic 8$\times$8 benchmark.

Second, Gymnasium-generated random maps are used as a diagnostic trial. They should be read differently from the standard Gymnasium map and from the controlled benchmark above. They answer a design question: what happens if we rely directly on the library's random-map generator at lower frozen-tile probabilities?

The answer is that performance drops sharply on many generated random maps, and the reason is visible from the oracle. Several $p=0.5$ and $p=0.6$ random maps have oracle success near zero under slippery dynamics. These maps are technically connected, because the generator guarantees a path from start to goal, but the stochastic transition model makes reliable completion extremely unlikely. Across all 13 Gymnasium maps, including the standard map and the random maps, optimistic Q-learning remains the best learned method, but its mean final success is only 0.235 because the near-impossible generated maps dominate the average. This result explains why the project moved toward carefully designed maps. It does not invalidate the standard Gymnasium 8$\times$8 result or the controlled-map results; it shows that random-map generation must be treated as an experimental variable.
...
\section{Discussion}
The main conclusion is that stochastic FrozenLake rewards exploration more than raw algorithmic complexity, but only after the benchmark maps are defined carefully. Optimistic Q-learning performs best on the controlled 8$\times$8 benchmark because optimistic initial values actively encourage the agent to try uncertain actions, which is crucial when the only positive reward is at the goal. The long-budget results also clarify an earlier concern: the large standard deviations in short-budget random-map plots mainly came from mixing maps with different layouts before the algorithms had enough time to converge. When each map is plotted separately and the 8$\times$8 budget is increased to 10,000 episodes, most tabular methods improve greatly on the controlled maps.
...
The map design results are important. A single fixed map is reproducible but not enough for validation, but some fixed maps are still essential standards. The Gymnasium standard 8$\times$8 map is one such case: it is a useful common reference and confirms that our algorithms can learn a recognized stochastic benchmark. Random maps are also valuable, but their role must be stated clearly. Our workflow reflects this: the Gymnasium random-map attempt exposed many near-zero oracle maps, so we moved to controlled maps for the main comparison. For 4$\times$4, maps need a controlled hole count, reachability checks, and oracle filtering; otherwise the benchmark can accidentally become too easy. For 8$\times$8, the controlled random maps are appropriate for comparing algorithms because they produce solvable stochastic tasks with meaningful variation. The Gymnasium random-map diagnostic remains valuable because it explains the boundary of the claim: lower-$p$ random maps may be connected in the deterministic sense but nearly impossible under slippery transitions.
...
\section{Conclusion}
We converted the original 4$\times$4 Q-learning project into a stochastic FrozenLake benchmark with multiple tabular algorithms, controlled random maps, Value Iteration oracle references, Optuna tuning, ablation studies, and long-budget per-map learning curves. The key methodological contribution is the benchmark-design refinement: the Gymnasium standard 8$\times$8 map provides an important standard comparison, but direct Gymnasium random maps were too often near-impossible under slippery dynamics, so controlled maps were introduced to make algorithm comparison meaningful. The central algorithmic result is that optimistic Q-learning is the strongest learned method on the controlled stochastic benchmark, achieving near-oracle performance on controlled random 8$\times$8 maps. Expected SARSA, SARSA, and Q-learning form a strong second group, while Dyna-Q is more attractive when learning speed and AUC matter. The final practical recommendation is therefore: use Q-learning as a baseline, add optimistic exploration, validate on standard maps such as Gymnasium 8$\times$8, compare algorithms on controlled stochastic maps, and use Gymnasium-generated maps plus oracle gaps as a diagnostic check before making broad claims about random-map generalization.
