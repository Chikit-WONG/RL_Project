...
Figure~\ref{fig:designed4} reports the controlled random 4$\times$4 experiment. These maps also have exactly four holes and are therefore much more credible than high-$p$ random maps. The results should be read as a credibility check rather than a strong ranking claim: Expected SARSA, Q-learning, and optimistic Q-learning are close in mean performance, but the standard deviations across only four maps are large. This means the controlled 4$\times$4 setting confirms that the algorithms behave plausibly on non-trivial small boards, while the more reliable algorithm ranking should come from the larger 8$\times$8 controlled benchmark.
