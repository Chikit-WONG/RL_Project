...
The experiment was developed in two stages. We first used Gymnasium as the reference implementation. The standard Gymnasium 8$\times$8 board is valuable because it is widely recognized, reproducible, and neither trivial nor impossible under \texttt{slippery=True}. It therefore serves as an important standard game benchmark and sanity check for the training pipeline.
...
\section{Discussion}
The main conclusion is that stochastic FrozenLake remains a game-playing problem, but the quality of the comparison depends strongly on the board set. The agent must learn a robust policy for reaching the goal under slips, not memorize a deterministic route. Under this objective, exploration matters more than raw algorithmic complexity, but only after the benchmark maps are defined carefully. Optimistic Q-learning performs best on the controlled 8$\times$8 benchmark because optimistic initial values actively encourage the agent to try uncertain actions, which is crucial when the only positive reward is at the goal. The long-budget results also clarify an earlier concern: the large standard deviations in short-budget random-map plots mainly came from mixing maps with different layouts before the algorithms had enough time to converge. When each map is plotted separately and the 8$\times$8 budget is increased to 10,000 episodes, most tabular methods improve greatly on the controlled maps.
